...
void Widget::init_widget()//初始化相关的控件
{
    ...
    layout2->addWidget(toolButton_5);

    lineEdit = new QLineEdit;

    pushButton = new QPushButton;
    pushButton->setText(tr("发送"));
    pushButton->setEnabled(false);//初始默认不可用
    ...
    connect(toolButton_5, SIGNAL(clicked()), this, SLOT(on_toolButton_5_clicked()));
    connect(lineEdit, SIGNAL(returnPressed()), this, SLOT(on_lineEdit_returnPressed()));
    connect(pushButton, SIGNAL(clicked()), this, SLOT(on_pushButton_clicked()));

    camera = new CameraMainWin(this);
}
...
void Widget::on_pushButton_clicked()
{
    ...
    else
    {
      ...
      //  QByteArray arr = lineEdit->text().toLatin1();
        QTextCodec* codec = QTextCodec::codecForName("utf-8");
        QByteArray arr =  codec->fromUnicode(lineEdit->text());
        main_w->send_Msg(userID, arr);//调用主窗口的send_Msg方法，向服务器提交send消息
...
{
    main_w->send_Request_Camera(userID);//调用主窗口的send_Msg_Camera方法，向服务器提交send消息
}
