/****************************************************************************
** Meta object code from reading C++ file 'toolbox1.h'
**
** Created: Thu Dec 29 13:00:11 2016
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "toolbox1.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'toolbox1.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_toolbox1[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      29,   15,   10,    9, 0x0a,
      67,   59,    9,    9, 0x08,
     108,    9,    9,    9, 0x08,
     119,    9,    9,    9, 0x08,
     138,    9,    9,    9, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_toolbox1[] = {
    "toolbox1\0\0bool\0watched,event\0"
    "eventFilter(QObject*,QEvent*)\0sockErr\0"
    "sock_Error(QAbstractSocket::SocketError)\0"
    "read_Msg()\0socket_connected()\0"
    "on_music_clicked()\0"
};

const QMetaObject toolbox1::staticMetaObject = {
    { &QToolBox::staticMetaObject, qt_meta_stringdata_toolbox1,
      qt_meta_data_toolbox1, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &toolbox1::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *toolbox1::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *toolbox1::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_toolbox1))
        return static_cast<void*>(const_cast< toolbox1*>(this));
    return QToolBox::qt_metacast(_clname);
}

int toolbox1::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QToolBox::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: { bool _r = eventFilter((*reinterpret_cast< QObject*(*)>(_a[1])),(*reinterpret_cast< QEvent*(*)>(_a[2])));
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = _r; }  break;
        case 1: sock_Error((*reinterpret_cast< QAbstractSocket::SocketError(*)>(_a[1]))); break;
        case 2: read_Msg(); break;
        case 3: socket_connected(); break;
        case 4: on_music_clicked(); break;
        default: ;
        }
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
